...
(function () {
    ...
    if (b.prototype.addCamera = function (a) {
        ...
        return f = new d(this.graph, e, this.settings, g), this.renderers[c] = f, Object.defineProperty(f, "id", {
            value: c
        }), f.bind && f.bind(["click", "clickStage", "doubleClickStage", "clickNode", "clickNodes", "doubleClickNode", "doubleClickNodes", "overNode", "overNodes", "outNode", "outNodes", "downNode", "downNodes", "upNode", "upNodes"], this._handler), this.renderersPerCamera[e.id].push(f), f
...
function (a) {
    ...
    sigma.utils = sigma.utils || {}, sigma.utils.extend = function () {
        ...
        a.addEventListener(b, function (a) {
            return d++, 2 === d ? (d = 0, c(a)) : (1 === d && setTimeout(function () {
                d = 0
            }, sigma.settings.doubleClickTimeout), void 0)
        }, !1)
...
function () {
    ...
    sigma.utils.pkg("sigma.captors"), sigma.captors.mouse = function (a, b, c) {
        ...
        function i(a) {
            var b, c, d, e;
            return w("mouseEnabled") ? (d = 1 / w("doubleClickZoomingRatio"), e = Math.max(w("zoomMin"), Math.min(w("zoomMax"), v.ratio * d)), d = e / v.ratio, t.dispatchEvent("doubleclick", {
                x: o - a.target.width / 2,
                y: p - a.target.height / 2
            }), w("doubleClickEnabled") && e !== v.ratio && (c = sigma.misc.animation.killAll(v), b = v.cameraPosition(sigma.utils.getX(a) - a.target.width / 2, sigma.utils.getY(a) - a.target.height / 2, !0), sigma.misc.animation.camera(v, {
                x: b.x * (1 - d) + v.x,
                y: b.y * (1 - d) + v.y,
                ratio: e
            }, {
...
function () {
    ...
    sigma.utils.pkg("sigma.captors"), sigma.captors.touch = function (a, b, c) {
        ...
        function h(a) {
            var b, c, e, f;
            return a.touches && 1 === a.touches.length && D("touchEnabled") ? (y = !0, e = 1 / D("doubleClickZoomingRatio"), f = Math.max(D("zoomMin"), Math.min(D("zoomMax"), C.ratio * e)), e = f / C.ratio, b = d(a.touches[0]), A.dispatchEvent("doubleclick", {
                x: b.x - a.target.width / 2,
                y: b.y - a.target.height / 2
            }), D("doubleClickEnabled") && f !== C.ratio && (c = sigma.misc.animation.killAll(C), b = C.cameraPosition(b.x - a.target.width / 2, b.y - a.target.height / 2, !0), sigma.misc.animation.camera(C, {
                x: b.x * (1 - e) + C.x,
                y: b.y * (1 - e) + C.y,
                ratio: f
            }, {
...
function (a) {
    ...
    sigma.utils.pkg("sigma.renderers"), sigma.renderers.canvas = function (a, b, c, d) {
        ...
        var c, d, e, f, g, h, i, j, k, l, m, n, o = {}, p = this.graph,
            q = this.graph.nodes,
            r = (this.options.prefix || "", this.settings(b, "drawEdges")),
            s = this.settings(b, "drawNodes"),
            t = this.settings(b, "drawLabels"),
            u = this.settings.embedObjects(b, {
                prefix: this.options.prefix
            });
        this.settings(b, "hideEdgesOnMove") && (this.camera.isAnimated || this.camera.isMoving) && (r = !1), this.camera.applyView(a, this.options.prefix, {
            width: this.width,
            height: this.height
        }), this.clear();
        ...
        if (r) {
            for (c = p.edges(), d = 0, f = c.length; f > d; d++) g = c[d], !o[g.source] && !o[g.target] || g.hidden || q(g.source).hidden || q(g.target).hidden || this.edgesOnScreen.push(g);
            if (this.settings(b, "batchEdgesDrawing")) h = "edges_" + this.conradId, n = u("canvasEdgesBatchSize"), l = this.edgesOnScreen, f = l.length, k = 0, i = Math.min(l.length, k + n), j = function () {
                for (m = sigma.canvas.edges, d = k; i > d; d++) g = l[d], (m[g.type] || m.def)(g, p.nodes(g.source), p.nodes(g.target), this.contexts.edges, u);
                return i === l.length ? (delete this.jobs[h], !1) : (k = i + 1, i = Math.min(l.length, k + n), !0)
            }, this.jobs[h] = j, conrad.addJob(h, j.bind(this));
            else
                for (m = sigma.canvas.edges, c = this.edgesOnScreen, d = 0, f = c.length; f > d; d++) g = c[d], (m[g.type] || m.def)(g, p.nodes(g.source), p.nodes(g.target), this.contexts.edges, u)
        }
        if (s)
            for (m = sigma.canvas.nodes, c = this.nodesOnScreen, d = 0, f = c.length; f > d; d++) c[d].hidden || (m[c[d].type] || m.def)(c[d], this.contexts.nodes, u);
        if (t)
            for (m = sigma.canvas.labels, c = this.nodesOnScreen, d = 0, f = c.length; f > d; d++) c[d].hidden || (m[c[d].type] || m.def)(c[d], this.contexts.labels, u);
...
function () {
    ...
    sigma.utils.pkg("sigma.misc"), sigma.misc.bindEvents = function (a) {
        ...
        function c(a) {
            function c(a) {
                h.settings("eventsEnabled") && (h.dispatchEvent("click", a.data), g = b(a), g.length ? (h.dispatchEvent("clickNode", {
                    node: g[0]
            ...
            function d(a) {
                h.settings("eventsEnabled") && (h.dispatchEvent("doubleClick", a.data), g = b(a), g.length ? (h.dispatchEvent("doubleClickNode", {
                    node: g[0]
            ...
            function e() {
                if (h.settings("eventsEnabled")) {
                    var a, b, c, d = [];
                    for (a in i) d.push(i[a]);
                    for (i = {}, b = 0, c = d.length; c > b; b++) h.dispatchEvent("outNode", {
                        node: d[b]
                    });
            ...
            function f(a) {
                if (h.settings("eventsEnabled")) {
                    ...
                    var c, d, e, f = [],
                        j = [],
                        k = {}, l = g.length;
                    for (c = 0; l > c; c++) e = g[c], k[e.id] = e, i[e.id] || (j.push(e), i[e.id] = e);
                    for (d in i) k[d] || (f.push(i[d]), delete i[d]);
            ...
            var g, i = {};
            a.bind("click", c), a.bind("mouseup", f), a.bind("mousemove", f), a.bind("mouseout", e), a.bind("doubleclick", d), h.bind("render", f)
        }
        var d, e, f, g, h = this;
...
function () {
    ...
    sigma.utils.pkg("sigma.misc"), sigma.misc.drawHovers = function (a) {
        function b() {
            ...
            if (f("enableHovering"))
                for (b in d) d[b].hidden || (e[d[b].type] || e.def)(d[b], c.contexts.hover, f)
        }
        var c = this,
            d = {};
